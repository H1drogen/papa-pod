...
{
    public class HolidayDal : IHolidayDal
    {
        private readonly string apiUrl = "/rpt/holidays";
        ...
        public HolidayDal(HttpClient httpClient,
        ...
        public async Task<IEnumerable<Holiday>> GetAllAsync()
        {
            ...
            return await response.Content.ReadAsAsync<IEnumerable<Holiday>>();
        }
        ...
        public async Task<Holiday> GetByIdAsync(int id)
        {
            ...
            return await response.Content.ReadAsAsync<Holiday>();
        }

        public async Task<Holiday> PostAsync(Holiday holiday)
        {
        ...
        public async Task<Holiday> PutAsync(Holiday holiday)
        {
            var json = JsonConvert.SerializeObject(holiday);
            var putContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");

            var response = await httpClientWrapper.PutAsync(httpClient, $"{route}/{holiday.Id}", putContent);

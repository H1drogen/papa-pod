...
{
    public class CourseModuleTemplateDal : ICourseModuleTemplateDal
    {
        private readonly string apiUrl = "/rpt/course-templates";
        ...
        public CourseModuleTemplateDal(HttpClient httpClient,
        ...
        public async Task<IEnumerable<CourseModuleTemplate>> GetAllAsync()
        {
            var response = await httpClientWrapper.GetAsync(httpClient, route);
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadAsAsync<IEnumerable<CourseModuleTemplate>>();
        }

        public async Task<CourseModuleTemplate> GetByIdAsync(int id)
        {
            var response = await httpClientWrapper.GetAsync(httpClient, $"{route}/{id}");
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadAsAsync<CourseModuleTemplate>();
        }

        public async Task<CourseModuleTemplate> PostAsync(CourseModuleTemplate courseModuleTemplate)
        {
            var json = JsonConvert.SerializeObject(courseModuleTemplate);
        ...
        public async Task<CourseModuleTemplate> PutAsync(CourseModuleTemplate courseModuleTemplate)
        {
            var json = JsonConvert.SerializeObject(courseModuleTemplate);
            var putContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");

            var response = await httpClientWrapper.PutAsync(httpClient, $"{route}/{courseModuleTemplate.Id}", putContent);

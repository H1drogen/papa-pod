...
{
    public class TrainerRoleDal : ITrainerRoleDal
    {
        private readonly string apiUrl = "/rpt/trainer-roles";
        ...
        public TrainerRoleDal(HttpClient httpClient,
        ...
        public async Task<IEnumerable<TrainerRole>> GetAllAsync()
        {
            var response = await httpClientWrapper.GetAsync(httpClient, route);
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadAsAsync<IEnumerable<TrainerRole>>();
        }

        public async Task<TrainerRole> GetByIdAsync(int id)
        {
            var response = await httpClientWrapper.GetAsync(httpClient, $"{route}/{id}");
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadAsAsync<TrainerRole>();
        }

        public async Task<TrainerRole> PostAsync(TrainerRole trainerRole)
        {
            var json = JsonConvert.SerializeObject(trainerRole);
        ...
        public async Task<TrainerRole> PutAsync(TrainerRole trainerRole)
        {
            var json = JsonConvert.SerializeObject(trainerRole);
            var putContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");

            var response = await httpClientWrapper.PutAsync(httpClient, $"{route}/{trainerRole.Id}", putContent);

...
{
    public class CourseTemplateDal : ICourseTemplateDal
    {
        private readonly string apiUrl = "/rpt/pathway-templates";
        ...
        public CourseTemplateDal(HttpClient httpClient,
        ...
        public async Task<IEnumerable<CourseTemplate>> GetAllAsync()
        {
            var response = await httpClientWrapper.GetAsync(httpClient, route);
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadAsAsync<IEnumerable<CourseTemplate>>();
        }
        ...
        public async Task<CourseTemplate> GetByIdAsync(int id)
        {
            ...
            return await response.Content.ReadAsAsync<CourseTemplate>();
        }

        public async Task<CourseTemplate> PostAsync(CourseTemplate courseTemplate)
        {
            var json = JsonConvert.SerializeObject(courseTemplate);
        ...
        public async Task<CourseTemplate> PutAsync(CourseTemplate courseTemplate)
        {
            var json = JsonConvert.SerializeObject(courseTemplate);
            var putContent = new StringContent(json, UnicodeEncoding.UTF8, "application/json");

            var response = await httpClientWrapper.PutAsync(httpClient, $"{route}/{courseTemplate.Id}", putContent);
